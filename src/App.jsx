    ...
    <div style={{ minHeight: "100vh", background: "#f2f6ff", fontFamily: "'Plus Jakarta Sans', sans-serif", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "28px 20px 0", flex: 1 }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 24 }}>
          {[["⚽","All Sports","Soccer, NBA, NFL, MLB, NHL, UFC"],["🔍","Real-Time Data","Web search for live form and injuries"],["🤖","AI Analysis","Claude AI generates full betting context"],["💰","Value Bets","Top value picks across all leagues"],["📊","Confidence Score","AI rates each bet 0 to 100"],["🏆","500+ Leagues","All major competitions worldwide"]].map(([icon,title,desc],i) => (
        ...
        <div style={{ background: "white", borderRadius: 24, padding: 24, border: "2px solid #0057ff", boxShadow: "0 8px 32px rgba(0,87,255,0.12)", marginBottom: 16 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 20 }}>
            {["Unlimited AI analyses","All sports and 500+ leagues","Real-time web search data","Full betting context reports","Confidence scores and value ratings","Red flags and risk analysis","Cancel anytime"].map((f,i) => (
...
export default function ReleyzApp() {
  ...
  const runAnalysis = async () => {
    ...
    } catch (err) { clearInterval(interval); setAnalysisResult({ error: err.message || "Analysis failed. Please try again." }); }
    ...
    <div style={{ minHeight: "100vh", background: "#f2f6ff", fontFamily: "'Plus Jakarta Sans', sans-serif", color: "#0a0f1e", position: "relative" }}>
      ...
      {activePage === "matches" && (
        <div style={{ paddingBottom: 90 }}>
          ...
          <div style={{ background: "linear-gradient(135deg,#0047dd 0%,#0088ee 60%,#00aaff 100%)", padding: "20px 16px", position: "relative", overflow: "hidden" }}>
            ...
            <div style={{ position: "relative" }}>
              ...
              <div style={{ fontFamily: "'Barlow Condensed'", fontSize: 30, fontWeight: 800, color: "white", lineHeight: 1.1, marginBottom: 16 }}>{activeLeague.name}<br />Upcoming Fixtures</div>
            ...
            {matches.map((m,i) => (
              <div key={i} className={`match-card ${selectedMatch === m ? "selected" : ""} fade-up`} style={{ animationDelay: `${i*0.05}s` }} onClick={() => { setSelectedMatch(m); setAnalysisResult(null); }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <div style={{ flex: 1, height: 4, background: "#eef0f8", borderRadius: 2, overflow: "hidden" }}><div style={{ width: `${m.conf}%`, height: "100%", background: "linear-gradient(90deg,#0047dd,#00aaff)", borderRadius: 2 }} /></div>
      ...
      {activePage === "analysis" && (
        <div style={{ padding: "16px 16px 100px" }} className="fade-up">
          ...
          {analysisResult && !analysisResult.error && selectedMatch && (
            <>
              <div ref={analysisRef} style={{ background: "linear-gradient(135deg,#0047dd,#0099ff)", borderRadius: 20, padding: 20, marginBottom: 16, position: "relative", overflow: "hidden" }}>
