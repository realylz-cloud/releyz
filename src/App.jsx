...
function FormationPitch({ teamData, side = "home" }) {
  ...
  return (
    <div style={{ background: pitchColor, borderRadius: 12, overflow: "hidden", position: "relative", padding: "8px 4px" }}>
      {/* Pitch markings */}
      <svg style={{ position: "absolute", inset: 0, width: "100%", height: "100%" }} viewBox="0 0 300 420" preserveAspectRatio="none">
        <rect x="10" y="10" width="280" height="400" fill="none" stroke={lineColor} strokeWidth="1.5"/>
        <line x1="10" y1="210" x2="290" y2="210" stroke={lineColor} strokeWidth="1"/>
        <circle cx="150" cy="210" r="40" fill="none" stroke={lineColor} strokeWidth="1"/>
        <circle cx="150" cy="210" r="2" fill={lineColor}/>
        <rect x="85" y="10" width="130" height="55" fill="none" stroke={lineColor} strokeWidth="1"/>
        <rect x="110" y="10" width="80" height="28" fill="none" stroke={lineColor} strokeWidth="1"/>
        <rect x="85" y="355" width="130" height="55" fill="none" stroke={lineColor} strokeWidth="1"/>
        <rect x="110" y="382" width="80" height="28" fill="none" stroke={lineColor} strokeWidth="1"/>
      </svg>

      {/* Team name + formation */}
      <div style={{ textAlign: "center", position: "relative", zIndex: 1, marginBottom: 6 }}>
        <span style={{ fontSize: 10, fontWeight: 800, color: "white", background: "rgba(0,0,0,.3)", padding: "2px 10px", borderRadius: 10 }}>{teamData.team} · {teamData.formation}</span>
      </div>

      {/* Players — render rows from GK at bottom to forwards at top if home, reversed if away */}
      <div style={{ position: "relative", zIndex: 1, padding: "4px 0", minHeight: 320 }}>
        {(isHome ? [...rows].reverse() : rows).map((rowKey, rowIdx) => {
          const rowPlayers = [...byRow[rowKey]].sort((a,b) => a.col - b.col);
          return (
            <div key={rowKey} style={{ display: "flex", justifyContent: "space-around", alignItems: "center", marginBottom: rowIdx < totalRows - 1 ? 12 : 0 }}>
              {rowPlayers.map((p, pi) => (
                <div key={pi} style={{ display: "flex", flexDirection: "column", alignItems: "center", maxWidth: 52, textAlign: "center" }}>
                  <div style={{ width: 30, height: 30, borderRadius: "50%", background: teamData.color || "#0057ff", border: "2px solid white", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 800, color: "white", boxShadow: "0 2px 6px rgba(0,0,0,.3)" }}>
                    {p.jersey || "?"}
                  </div>
                  <div style={{ fontSize: 8, color: "white", fontWeight: 700, marginTop: 3, lineHeight: 1.2, textShadow: "0 1px 2px rgba(0,0,0,.8)", maxWidth: 50, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {p.name.split(" ").slice(-1)[0]}
                  </div>
                  <div style={{ fontSize: 7, color: "rgba(255,255,255,.7)", lineHeight: 1 }}>{p.position}</div>
                </div>
              ))}
            </div>
          );
      ...
      {teamData.manager && (
        <div style={{ textAlign: "center", position: "relative", zIndex: 1, marginTop: 4 }}>
          <span style={{ fontSize: 9, color: "rgba(255,255,255,.6)" }}>🧑‍💼 {teamData.manager}</span>
        </div>
      )}
    </div>
  );
    ...
    <div>
      ...
      {(activeTeam === "both" || activeTeam === "home") && data.home && (
        <div style={{ marginBottom: 16 }}>
          <FormationPitch teamData={data.home} side="home" />
        </div>
      )}
      {(activeTeam === "both" || activeTeam === "away") && data.away && (
        <div style={{ marginBottom: 16 }}>
          <FormationPitch teamData={data.away} side="away" />
        </div>
      )}
            ...
            {data.home.bench.map((p, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, background: "#f5f7ff", borderRadius: 8, padding: "5px 10px" }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 9, color: "#889" }}>{p.position}</div>
                </div>
              </div>
            ))}
          </div>
...
function MatchDetailModal({ game, leagueSlug, onClose }) {
  ...
  const hasLineups = detail?.homeLineup?.length > 0 || detail?.awayLineup?.length > 0;
  const hasStats = detail?.homeStats && Object.keys(detail.homeStats).length > 0;
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(10,15,30,.65)", zIndex: 200, display: "flex", alignItems: "flex-end", backdropFilter: "blur(4px)" }} onClick={onClose}>
      <div style={{ background: "white", borderRadius: "20px 20px 0 0", width: "100%", maxHeight: "90vh", overflow: "hidden", display: "flex", flexDirection: "column" }} onClick={e => e.stopPropagation()}>
        {/* Handle */}
        ...
        <div style={{ padding: "10px 16px 12px", borderBottom: "1px solid #f0f2ff" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
            <div style={{ fontSize: 10, color: "#889", fontWeight: 600 }}>{game.date} · {game.competition}</div>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <span style={{ fontSize: 13, fontWeight: 800, lineHeight: 1.2 }}>{game.homeTeam}</span>
            </div>
            ...
            <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 8, justifyContent: "flex-end" }}>
              <span style={{ fontSize: 13, fontWeight: 800, textAlign: "right", lineHeight: 1.2 }}>{game.awayTeam}</span>
              <TeamLogo logo={game.awayLogo} name={game.awayTeam} size={28} />
            </div>
          </div>
          {detail?.venue && <div style={{ fontSize: 10, color: "#889", textAlign: "center", marginTop: 6 }}>📍 {detail.venue}{detail?.attendance ? ` · 👥 ${Number(detail.attendance).toLocaleString()}` : ""}</div>}
        </div>

        {/* Tab bar */}
        {!loading && detail && (
          <div style={{ display: "flex", gap: 0, borderBottom: "1px solid #f0f2ff", overflowX: "auto", flexShrink: 0 }}>
            {[["timeline","⚽ Goals"], hasStats && ["stats","📊 Stats"], hasLineups && ["lineups","👕 Lineups"]].filter(Boolean).map(([id, label]) => (
              <button key={id} onClick={() => setTab(id)} style={{ fontSize: 11, fontWeight: 700, padding: "10px 14px", border: "none", background: "white", color: tab === id ? "#0057ff" : "#889", borderBottom: tab === id ? "2px solid #0057ff" : "2px solid transparent", cursor: "pointer", whiteSpace: "nowrap", flexShrink: 0 }}>
                {label}
              </button>
            ))}
          </div>
        ...
        <div style={{ overflowY: "auto", flex: 1, padding: "16px" }}>
          ...
          {!loading && detail && (
            <>
              ...
              {tab === "timeline" && (
                <>
                  {detail.timeline?.length > 0 ? (
                            ...
                            {e.isHome ? (
                              <>
                                <div style={{ flex: 1, textAlign: "right", paddingRight: 14 }}>
                                  <div style={{ fontSize: 12, fontWeight: 800 }}>{e.player}</div>
                                  {e.assist && <div style={{ fontSize: 10, color: "#889" }}>Assist: {e.assist}</div>}
                                  <div style={{ fontSize: 11, color: "#0057ff", fontWeight: 700, marginTop: 2 }}>{e.homeScore} - {e.awayScore}</div>
                                </div>
                                <div style={{ width: 36, height: 36, borderRadius: "50%", background: "linear-gradient(135deg,#0047dd,#0099ff)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, zIndex: 1, boxShadow: "0 2px 8px rgba(0,87,255,.35)" }}>
                                  <span style={{ fontSize: 9, fontWeight: 800, color: "white", textAlign: "center" }}>{e.minute}</span>
                                </div>
                              ...
                              <>
                                ...
                                <div style={{ flex: 1, paddingLeft: 14 }}>
                                  <div style={{ fontSize: 12, fontWeight: 800 }}>{e.player}</div>
                                  {e.assist && <div style={{ fontSize: 10, color: "#889" }}>Assist: {e.assist}</div>}
                                  <div style={{ fontSize: 11, color: "#ff4444", fontWeight: 700, marginTop: 2 }}>{e.homeScore} - {e.awayScore}</div>
                                </div>
                              </>
                            )}
                          </div>
                    ...
                    <div style={{ textAlign: "center", padding: "30px 0", color: "#889", fontSize: 12 }}>No goal details available.</div>
                  )}
                </>
              )}
              ...
              {tab === "stats" && (
                <>
                  <div style={{ fontFamily: "'Barlow Condensed'", fontSize: 16, fontWeight: 700, marginBottom: 14 }}>📊 Match Stats</div>
                  {Object.entries(detail.homeStats || {}).map(([key, homeVal], i) => {
                    const awayVal = detail.awayStats?.[key];
                    if (!awayVal && awayVal !== 0) return null;
                    ...
                    return (
                      <div key={i} style={{ marginBottom: 14 }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 5 }}>
                          <span style={{ fontSize: 13, fontWeight: 800, color: "#0057ff" }}>{homeVal}</span>
                          <span style={{ fontSize: 10, color: "#889", fontWeight: 600 }}>{label}</span>
                          <span style={{ fontSize: 13, fontWeight: 800, color: "#cc0000" }}>{awayVal}</span>
                        </div>
                        <div style={{ display: "flex", height: 6, borderRadius: 3, overflow: "hidden", background: "#f0f2ff" }}>
                          <div style={{ width: `${(hNum/total)*100}%`, background: "linear-gradient(90deg,#0047dd,#0088ff)", borderRadius: "3px 0 0 3px" }} />
              ...
              {tab === "lineups" && (
                <>
                  <div style={{ fontFamily: "'Barlow Condensed'", fontSize: 16, fontWeight: 700, marginBottom: 14 }}>👕 Lineups</div>
                  {/* Home starters */}
                  {detail.homeLineup?.filter(p => p.starter).length > 0 && (
                    <div style={{ marginBottom: 16 }}>
                      <div style={{ fontSize: 11, fontWeight: 700, color: "#0057ff", marginBottom: 8 }}>{detail.home} — Starters</div>
                      {detail.homeLineup.filter(p => p.starter).map((p, i) => (
                        <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 0", borderBottom: "1px solid #f0f2ff" }}>
                          <div style={{ width: 24, height: 24, borderRadius: "50%", background: "#0057ff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9, fontWeight: 800, color: "white", flexShrink: 0 }}>{p.jersey || i+1}</div>
                          <div style={{ flex: 1 }}>
                            <span style={{ fontSize: 12, fontWeight: 700 }}>{p.name}</span>
                            {p.subOut && <span style={{ fontSize: 9, color: "#ff4444", marginLeft: 6 }}>↕ {p.subMinute || "?"}'</span>}
                          </div>
                          <span style={{ fontSize: 10, color: "#889", fontWeight: 600 }}>{p.position}</span>
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  {detail.homeLineup?.filter(p => !p.starter).length > 0 && (
                    <div style={{ marginBottom: 16 }}>
                      <div style={{ fontSize: 11, fontWeight: 700, color: "#889", marginBottom: 8 }}>{detail.home} — Bench</div>
                      {detail.homeLineup.filter(p => !p.starter).map((p, i) => (
                        <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 0", borderBottom: "1px solid #f0f2ff" }}>
                          <div style={{ width: 24, height: 24, borderRadius: "50%", background: "#eef3ff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9, fontWeight: 800, color: "#0057ff", flexShrink: 0 }}>{p.jersey || "?"}</div>
                          <div style={{ flex: 1 }}>
                            <span style={{ fontSize: 12, fontWeight: 600, color: "#556" }}>{p.name}</span>
                            {p.subIn && <span style={{ fontSize: 9, color: "#00aa44", marginLeft: 6 }}>↑ {p.subMinute || "?"}'</span>}
                          </div>
                          <span style={{ fontSize: 10, color: "#aab", fontWeight: 600 }}>{p.position}</span>
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  {detail.awayLineup?.filter(p => p.starter).length > 0 && (
                    <div style={{ marginBottom: 16 }}>
                      <div style={{ fontSize: 11, fontWeight: 700, color: "#cc0000", marginBottom: 8 }}>{detail.away} — Starters</div>
                      {detail.awayLineup.filter(p => p.starter).map((p, i) => (
                        <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 0", borderBottom: "1px solid #f0f2ff" }}>
                          <div style={{ width: 24, height: 24, borderRadius: "50%", background: "#cc0000", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9, fontWeight: 800, color: "white", flexShrink: 0 }}>{p.jersey || i+1}</div>
                          <div style={{ flex: 1 }}>
                            <span style={{ fontSize: 12, fontWeight: 700 }}>{p.name}</span>
                            {p.subOut && <span style={{ fontSize: 9, color: "#ff4444", marginLeft: 6 }}>↕ {p.subMinute || "?"}'</span>}
                          </div>
                  ...
                  {detail.awayLineup?.filter(p => !p.starter).length > 0 && (
                    <div>
                      <div style={{ fontSize: 11, fontWeight: 700, color: "#889", marginBottom: 8 }}>{detail.away} — Bench</div>
                      {detail.awayLineup.filter(p => !p.starter).map((p, i) => (
                        <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 0", borderBottom: "1px solid #f0f2ff" }}>
                          <div style={{ width: 24, height: 24, borderRadius: "50%", background: "#fff0f0", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9, fontWeight: 800, color: "#cc0000", flexShrink: 0 }}>{p.jersey || "?"}</div>
                          <div style={{ flex: 1 }}>
                            <span style={{ fontSize: 12, fontWeight: 600, color: "#556" }}>{p.name}</span>
                            {p.subIn && <span style={{ fontSize: 9, color: "#00aa44", marginLeft: 6 }}>↑ {p.subMinute || "?"}'</span>}
                          </div>
                  ...
                  {!hasLineups && <div style={{ textAlign: "center", padding: "30px 0", color: "#889", fontSize: 12 }}>Lineup data not available for this match.</div>}
                </>
              )}
            </>
          )}
...
function H2HSection({ match, leagueSlug }) {
  ...
  if (!data || (data.homeForm?.length === 0 && data.awayForm?.length === 0 && data.h2h?.length === 0)) return (
    <div style={{ textAlign: "center", padding: "50px 20px", color: "#889" }}>
      <div style={{ fontSize: 32, marginBottom: 12 }}>⚔️</div>
...
export default function ReleyzApp() {
  ...
  const runAnalysis = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/analyze`, { method:"POST", headers:{"Content-Type":"application/json","Authorization":`Bearer ${userToken}`}, body:JSON.stringify({match:selectedMatch,league:activeLeague,sport:activeSport,analysisTypes:activeAnalysis}) });
      const data = await res.json();
        ...
        <div style={{ padding:"16px 16px 100px" }} className="fade-up">
          ...
          {analysisResult && !analysisResult.error && selectedMatch && (
            <>
              ...
              {/* Three tabs: Overview, H2H, Lineups */}
